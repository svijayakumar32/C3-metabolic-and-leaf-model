...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%function gpmain_fix_potato_260(Ci, CO2_fix_260_potato)
global Vrubusco_adj;
...
%         gp2V111 = pop(3,j) * 0.24;            % Here the rate of Rubisco oxygenation is adjusted.
    ...
    %xlim([1, i+1]); ylim([0,80]);  
	  ...
	  if rem(i/100,1)==0
        i
        save(CO2_fix_potato_260);     % Save the work space
        %Save matrix of optimal enzyme rates to output file
%         file=fopen('outputenz_129_rice.txt','w');
%         fprintf(file,'%.4f\n',BestMatrix);
          writematrix(BestMatrix,'outputenz_fix_260_potato.txt');
          writematrix(d_plot,'d_plot_fix_260_potato.xls');
          %save CO2_129_potato_260;
      end
	%Save workspace
	%save output_potato_129_260;
end