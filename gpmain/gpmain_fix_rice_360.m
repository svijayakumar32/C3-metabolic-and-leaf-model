...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%function gpmain_fix_rice_360(Ci, CO2_fix_360_rice)
global Vrubusco_adj;
Vrubusco_adj=1.0; %keep as 1.0 after fixed optimization
global VmaxAdj;%
VmaxAdj=1.0;

%Ci=360;% adj 200,220,240 or 260
CO2i=360;% adj 200,220,240 or 260 %CO2i=Ci
PPFDi=2000;
WeatherTemp=25;
GRNC=0;
...
%         gp2V111 = pop(3,j) * 0.24;            % Here the rate of Rubisco oxygenation is adjusted.
    ...
    %xlim([1, i+1]); ylim([0,80]);  
	  ...
	  if rem(i/100,1)==0
        i
        save(CO2_129_rice_360);     % Save the work space
        %Save matrix of optimal enzyme rates to output file
%         file=fopen('outputenz_129_rice.txt','w');
%         fprintf(file,'%.4f\n',BestMatrix);
          writematrix(BestMatrix,'outputenz_129_360.txt');
          writematrix(d_plot,'d_plot_129_360.xls');
          %save CO2_129_rice_360;
    end
	%Save workspace
