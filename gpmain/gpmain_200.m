...
%WARNING population size (PopSize) must be dividible by 4
popSize = 16;       
numofGen = 500;       % The total number of generations    
mutatePercentage = 0.02;     % Maximal percentage of changes in Vmax in each generation

% Different options of  generationg new population. Default is 1. 
...
sumd = sumd- pop(9,1)/BK(7)*MW(7)-pop(11,1)/BK(9)*MW(9);      % The transketolse is double counted. So, it is corrected at here. 
...
CO2PLOT = zeros(3,5);  
% 1: Geneartion number; 2: Average; 3, Error bar; 
...
%         gp2V111 = pop(3,j) * 0.24;            % Here the rate of Rubisco oxygenation is adjusted.
            ...
            ddiff=d_plot(sizeT(1),53:87)-d_plot(IndexMiddle,53:87);%matabolits
            dnorm1 = norm(ddiff);
    ...
    xlim([1, i+1]); ylim([0,80]);  
        ...
        save CO2_200_1;     % Save the work space
        %save output;
end
