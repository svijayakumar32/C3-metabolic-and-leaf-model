...
global Vrubusco_adj;
Vrubusco_adj=1.12;%potato = 0.7, rice = 1.12 %keep as 1.0 after fixed optimization
global VmaxAdj;%
VmaxAdj=1.24;%potato = 2.1, rice = 1.24

CO2i=176;% adj values 200,220,240 or 260
PPFDi=2000;
WeatherTemp=25;
GRNC=0;
Einput=ones(37,1);%No gene expression data input
%Edata=importdata('Einput7.txt');
Edata=importdata('Einput_rice_new.txt');
Eio=Edata.data(:,1);
...
%         gp2V111 = pop(3,j) * 0.24;            % Here the rate of Rubisco oxygenation is adjusted.
    ...
    %xlim([1, i+1]); ylim([0,80]);  
	  ...
	  if rem(i/100,1)==0
        ...
        %file=fopen('outputenz_176_rice.txt','w');%Rename output file according to Ci and crop name
        %fprintf(file,'%.4f\n',BestMatrix);
        writematrix(BestMatrix,'outputenz_176_rice_new.txt');
        writematrix(d_plot,'d_plot_176_rice_new.xls');
        save CO2_176_rice_new
    end
	%Save workspace
    %save output_rice_176_new;
    end
