...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%function gpmain_fix_rice_300(Ci, CO2_fix_300_rice)
global Vrubusco_adj;
Vrubusco_adj=1.0; %keep as 1.0 after fixed optimization
global VmaxAdj;%
VmaxAdj=1.0;

%Ci=300;% adj 200,220,240 or 260
CO2i=300;% adj 200,220,240 or 260 %CO2i=Ci
PPFDi=2000;
WeatherTemp=25;
GRNC=0;
Einput=ones(37,1);%No gene expression data input
%Edata=importdata('Einput7.txt');
Edata=importdata('Einput_rice_129.txt');
Eio=Edata.data(:,1);
...
%         gp2V111 = pop(3,j) * 0.24;            % Here the rate of Rubisco oxygenation is adjusted.
    ...
    %xlim([1, i+1]); ylim([0,80]);  
	  ...
	  if rem(i/100,1)==0
        i
        save(CO2_129_rice_300);     % Save the work space
        %Save matrix of optimal enzyme rates to output file
%         file=fopen('outputenz_129_rice.txt','w');
%         fprintf(file,'%.4f\n',BestMatrix);
          writematrix(BestMatrix,'outputenz_129_300.txt');
          writematrix(d_plot,'d_plot_129_300.xls');
          %save CO2_129_rice_300;
    end
	%Save workspace
