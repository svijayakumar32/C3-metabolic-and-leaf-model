...
Edata=importdata('Einput_potato.txt');
Eio=Edata.data(:,1);
...
pop = mutate_w(pop, popSize, mutatePercentage);
...
%           Eiopop(12)=Eio(12);%VATP synthesis not used
            ...
            %from adjusted enzyme levels in Einput_potato but missing out V8, V10 and V16 from optimization
...
%             Temp=EPS_Drive_GRNs(Einput,CO2i,PPFDi,WeatherTemp,GRNC,0,Eiopop);
    ...
    %xlim([1, i+1]); ylim([0,80]);  
        ...
        workspacefileName = strcat ("CO2_potato_",Ci_str,"_New_1.mat");
        % Save the work space 
        save(workspacefileName);
        %Save matrix of optimal enzyme rates to output file
        BestMatrix=BestMatrix'; % Transpose matrix
        BestMatrixfileName = strcat ("outputenz_",Ci_str,"_New_potato_1.txt");
        d_plotfileName = strcat ("d_plot_",Ci_str,"_New_potato_1.xls");
